...
anu = await fetchJson(`https://ryuu-apii.herokuapp.com/api/download/ytmp3?url=${url}&apikey=RyuBotz`)
...
deff.sendMessage(from, ytmp3, audio, {mimetype:"audio/mp4", quoted:mek})
limitAdd(sender, limit)
break;
case 'ytmp4':
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(mess.limit)
...
url = args.join(' ')
reply(`Tunggu Butuh Beberapa Menit!`)
anu = await fetchJson(`https://ryuu-apii.herokuapp.com/api/download/ytmp4?url=${url}&apikey=RyuBotz`)
ytmp4 = await getBuffer(anu.result.url)
reply(`_*Data Di DapatKan!!*_\n\n💬 *Title* : ${anu.result.title}\n🖼 *Thumb* : ${anu.result.thumb}\n🖥 *Channel* : ${anu.result.channel}\n📅 *Publish* : ${anu.result.published}\n👁 *Viewers* : ${anu.result.views}\n\nAudio Akan Dikirim Beberapa Menit!`)
deff.sendMessage(from, ytmp4, video, {caption:`Done By : ${botName}`, thumbnail:Buffer.alloc(0), quoted:mek})
limitAdd(sender, limit)
break;
